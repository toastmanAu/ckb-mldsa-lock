 ...
 * for testing the CKB ML-DSA-44 lock script.
 *
 * Build:
...
/* ── sizes ────────────────────────────────────────────────────────── */
#define MLDSA65_PUBLICKEY_BYTES  1312
#define MLDSA65_SIGNATURE_BYTES  2420
#define MLDSA65_SECRETKEY_BYTES  2560
...
int main(int argc, char **argv) {
    ...
    int rc = PQCP_MLDSA_NATIVE_MLDSA44_keypair(pk, sk);
    ...
    size_t  siglen = 0;
    rc = PQCP_MLDSA_NATIVE_MLDSA44_signature(sig, &siglen, msg, 32, ctx, CKB_MLDSA_DOMAIN_LEN, sk);
    ...
    if (!expect_fail) {
        ...
        int vrc = PQCP_MLDSA_NATIVE_MLDSA44_verify(sig, siglen, msg, 32,
    ...
    serialize_mldsa_witness(mldsa_wit, 0x01, 0x02, 0x01, 0x00, pk, sig);
    ...
    lock_args[2] = 0x01;  /* param_id = ML-DSA-44 */  /* param_id = ML-DSA-44 */
