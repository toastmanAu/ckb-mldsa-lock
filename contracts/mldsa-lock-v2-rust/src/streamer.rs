//! Streamed CighashAll digest for on-chain ML-DSA verification.
//!
//! **Copied verbatim** from `ckb-fips204-utils::ckb_tx_message_all_in_ckb_vm`
//! (which itself was copied from `ckb-fips205-utils`, which was copied from
//! `xxuejie/ckb-tx-message-all-test-vector-utils` commit
//! `07495da75d4c048cbf8e08e630cbb27b8d3b5702`). The algorithm is
//! signature-scheme-agnostic — only the error type is local.
//!
//! Any change to this file MUST be mirrored in the siblings or the two
//! binaries will silently disagree on what bytes get hashed, which would
//! surface as `SignatureVerifyFailed` at spend time.
//!
//! # Why streamed
//!
//! CKB-VM instances have ~4 MB total memory, shared by code and data.
//! A single witness can be as large as a block (~600 KB), so loading any
//! piece of transaction data fully into memory is a non-starter. Every
//! input cell, cell data, and non-group-input witness is loaded in
//! 32 KB batches and hashed incrementally via `load_and_hash`.
//!
//! The exception is the **first witness of the current script group**: we
//! must validate its `WitnessArgs` structure before extracting the lock
//! field, which requires a single contiguous buffer.
...
/// Generate the CighashAll message for the current script-group's first
/// witness. Loads the first witness of `GroupInput` as a contiguous buffer
/// (required to validate the `WitnessArgs` structure) then streams the rest.
...
/// Streaming variant that takes the already-loaded first witness as a slice.
pub fn generate_ckb_tx_message_all_with_witness<W: io::Write>(
    writer: &mut W,
    first_witness_data: &[u8],
) -> Result<(), CkbTxMessageAllError> {
    // Validates that the first witness is of WitnessArgs structure
